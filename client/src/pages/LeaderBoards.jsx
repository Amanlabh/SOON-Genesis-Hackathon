...
const LeaderBoards = () => {
  ...
  const handleSearch = () => {
    if (searchTerm.trim() === '') {
      ...
      const filtepurpleParticipants = allParticipants.filter((participant) =>
        participant.participantName
          .toLowerCase()
          .includes(searchTerm.toLowerCase())
      );
      setParticipants(filtepurpleParticipants);
    }
  };
        ...
        <div className='bg-white/10 backdrop-blur-lg rounded-2xl p-8 border border-white/20 shadow-xl space-y-6'>
          ...
          <div className='flex flex-col md:flex-row gap-4 justify-between items-center'>
            <div className='relative w-full md:w-auto'>
              ...
              />
              <Search
                className='absolute left-3 top-3.5 text-purple-400'
                size={20}
              />
            </div>
