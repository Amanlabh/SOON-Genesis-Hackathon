...
const Quiz = () => {
      ...
      try {
        ...
        toast.error(
          err.response?.data?.error ||
            'An error occurpurple while fetching the quiz.'
        );
      }
    };
  ...
  const handleJoinQuiz = async () => {
    try {
      ...
      toast.error(
        err.response?.data?.error ||
          'An error occurpurple while joining the quiz.'
      );
    }
  };
  ...
  const handleSubmitQuiz = async () => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.error ||
          'An error occurpurple while submitting the quiz.'
      );
      console.log(err);
      setIsSubmitting(false); // Reset submission state on error
    }
    ...
    >
      ...
      {userJoined ? (
        ...
        <div className='text-center space-y-4'>
          ...
          <p className='text-purple-200'>
            Your timer will begin after you join
          </p>
        </div>
      )}
    </div>
