...
exports.createQuizByPrompt = async (req, res) => {    
    const { creatorName, creatorWallet, prompt, numParticipants, questionCount, rewardPerScore, isPublic } = req.body; 
...
exports.createQuizByPdf = async (req, res) => {
    const { creatorName, creatorWallet, numParticipants, questionCount, rewardPerScore, isPublic, totalCost } = req.body;
...
exports.getQuiz = async (req, res) => {
    ...
    try {
        ...
        const existingParticipant = await Participant.findOne({ quizId, walletAddress });
        if (existingParticipant) {
            return res.status(403).json({ error: 'You have already participated in this quiz.' });
        }

        const participantCount = await Participant.countDocuments({ quizId });
        if (participantCount >= quiz.numParticipants) {
            return res.status(403).json({ error: 'The number of participants for this quiz has been reached.' });
        }
...
exports.joinQuiz = async (req, res) => {
    ...
    try {
        ...
        const participant = new Participant({ quizId, participantName, walletAddress });
